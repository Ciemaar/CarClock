...
This project is an Android Automotive OS standalone app (API 33+) written in Kotlin using Jetpack Compose. It runs natively on the infotainment unit, *not* as an Android Auto phone projection.
...
Because this app relies on the `android.hardware.type.automotive` hardware constraint, it will *fail* to install on a regular Android phone emulator.
